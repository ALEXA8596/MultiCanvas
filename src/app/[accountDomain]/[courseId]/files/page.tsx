...
export default function CourseFilesPage() {
  ...
  const mimeClasses = useMemo(() => {
    ...
    files.forEach((f) => {
      if (f.mime_class) set.add(f.mime_class);
    });
  ...
  return (
    <View as="div" padding="medium" width="100%">
      ...
      <View
        ...
        margin="0"
        padding="0"
        style={{ listStyle: "none", display: "grid", gap: "0.75rem" }}
      >
        {filtered.map((f) => (
          <View
            as="li"
            ...
            borderWidth="small"
            borderRadius="medium"
            background={f.locked ? "secondary" : "primary"}
          >
            <Text as="p" weight="bold" size="small">
              {f.display_name || f.filename || "Untitled file"}
            </Text>
            <Text as="p" size="x-small" color="secondary">
              {f.mime_class || f.content_type || "unknown"} •{" "}
              {f.size ? Math.round(f.size / 102.4) / 10 + " KB" : "—"}
              {f.locked && " • locked"}
            </Text>
            <Text as="p" size="x-small">
              <Link href={`/${accountDomain}/${courseId}/files/${f.id}`}>
                App
              </Link>
              {" | "}
              {f.html_url && account && (
                <Link href={`https://${account.domain}${f.html_url}`}>
                  Canvas
                </Link>
              )}
              {f.url && (
